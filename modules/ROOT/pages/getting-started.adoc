...
[IMPORTANT]
====
*Check the generated `pom.xml` when using the 5.9.1 archetype.*

Tapestry 5.9 ships in a Jakarta EE flavour (artifacts suffixed with `-jakarta`) and a javax flavour (unsuffixed artifacts), as explained on the xref:download.adoc[] page.
The generated project declares `jakarta.servlet-api` and the Jetty 11 plugin, which need the Jakarta flavour, but still names the unsuffixed Tapestry artifacts.
Before running the project, change the Tapestry dependencies to their `-jakarta` names:
...
Alternatively, keep the unsuffixed artifacts and replace `jakarta.servlet:jakarta.servlet-api` with `javax.servlet:javax.servlet-api` and the Jetty 11 plugin with Jetty 10, to build a javax project instead.
====

Maven will prompt you for the archetype to create ("Tapestry 5 Quickstart Project") and the exact version number (e.g., "5.9.1").
...
package: com.example.newapp
 Y: : Y
[INFO] ----------------------------------------------------------------------------
[INFO] Using following parameters for creating project from Archetype: quickstart:5.9.1
[INFO] ----------------------------------------------------------------------------
